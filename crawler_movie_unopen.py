...
# 抓開眼電影
movie_data = []
res = requests.get("http://www.atmovie.com.tw/movie/next/0/")
res.encoding = 'utf8'
...
for item in soup.select(".filmListAll"):
    for item2 in item.select("li .filmtitle"):

        # 抓電影基本資料
        movie_title = item2.find('a').text
        movie_url = "http://www.atmovies.com.tw"+item2.find('a')['href']
        movie_id = item2.find('a')['href'][7:19]
        ...
        # 片長
        runtime = "" if "期" in soup2[2].select('.runtime li')[0].text[3:].split("分")[0] else soup2[2].select('.runtime li')[0].text[3:].split("分")[0]

        # 上映日期
        open_date = "" if len(soup2[2].select('.runtime li')) < 2 else soup2[2].select('.runtime li')[1].text[5:]

        if "期" in soup2[2].select('.runtime li')[0].text[3:].split("分")[0]:
            open_date = soup2[2].select('.runtime li')[0].text[3:].split("分")[0][2:]
